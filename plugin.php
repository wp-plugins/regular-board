...
$regular_board_version = '1.13.8.4-stable-final-public';

register_activation_hook ( __FILE__, 'regular_board_installation_option' );
if ( !function_exists ( 'regular_board_installation_option' ) ) {
	function regular_board_installation_option() {
		...
		$wpdb->query ( "ALTER TABLE $regular_board_users ADD user_chanmode BIGINT(20) NOT NULL AFTER user_colormode" );
		...
		$wpdb->query ( "ALTER TABLE $regular_board_posts ADD post_guestip TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci AFTER post_reply_count" );
		add_option ( 'regular_board_installation', 0 );
		delete_option ( 'regular_board_postingoptions' );
	}
}

$regular_board_posts_select  = 'post_id, post_parent, post_name, post_date, post_email, post_title, post_comment, post_comment_parent, post_type, post_url, post_board, post_moderator, post_last, post_sticky, post_locked, post_password, post_userid, post_public, post_report, post_reportcount, post_reply_count, post_guestip';
$regular_board_users_select  = 'user_id, user_date, user_ip, user_name, user_email, user_password, user_heaven, user_boards, user_follow, user_avatar, user_slogan, user_posts, user_level, user_strikes, user_logged_in, user_logged_in_from, user_colormode, user_chanmode';
...
if ( get_option ( 'regular_board_ascii' ) ) {
	if ( !function_exists ( 'regular_board_ascii' ) ) {
