					...
					if ( !$_REQUEST['COMMENT'] && !$URL ) {
						...
						if ( !$IS_IT_SPAM ) {
							...
							if ( !$post_comment && $post_url ) {
								$get_duplicate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $regular_board_posts WHERE post_url = %s", $post_url ) );
							}
							if ( $post_comment && !$post_url ) {
								$get_duplicate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $regular_board_posts WHERE post_comment = %s", $post_comment ) );
							}
							if ( $post_comment && $post_url ) {
								$get_duplicate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $regular_board_posts WHERE (post_comment = %s OR post_url = %s)", $post_comment, $post_url ) );
							}
